...
RESERVED_FUNCS = ['StrNCat', 'getaddrinfo', '_ui64toa', 'fclose', 'pthread_mutex_lock', 'gets_s', 'sleep',
...
def code_tokenize(code):
    ...
    new_code = []
    for token in code:
        new_code += to_camelcase(token).split('_')
    # filter
    new_code = [n for n in new_code if n not in ['{', '}', ';', ':']]
    return new_code


def symbolic_tokenize(code):
    def func_or_keyword(token):
        nonlocal symbol_table, func_count
        if token in KEYWORDS:
            return token
        if token not in symbol_table.keys():
            symbol_table[token] = "FUNC" + str(func_count)
            func_count += 1
        return symbol_table[token]

    def var_or_keyword(token):
        nonlocal symbol_table, var_count
        if token not in symbol_table.keys():
            symbol_table[token] = "VAR" + str(var_count)
            var_count += 1
        return symbol_table[token]

    code = remove_comments(code)
    code = re.sub('(\n)|(\\\\n)|(\\\\)|(\\t)|(/)|(\\r)', '', code)
    tokens = [token.strip() for token in word_tokenize(code) if token.strip() != '']

    func_count, var_count = 0, 0
    tokens = []
    symbol_table = {}
    ...
    for i in range(len(tokens)):
        token = tokens[i]
        # reserved tokens
        if token in KEYWORDS + RESERVED_FUNCS + PUNCTUATIONS:
            tokens.append(token)
        # literals
        elif re.match("^\"*\"$", token) is not None:
            tokens.append("STRING")
        elif re.match("^[0-9]+(\\.[0-9]+)?$", token) is not None or \
                re.match("^[0-9]*(\\.[0-9]+)$", token) is not None:
            tokens.append("NUMBER")
        # functions
        elif tokens[i + 1] == '(':
            i += 1
            tokens.append(func_or_keyword(token))
        elif token.endswith('('):
            tokens.append(func_or_keyword(token[:-1]) + '(')
        elif token.endswith('()'):
            tokens.append(func_or_keyword(token[:-2]) + '()')
        # variables
        else:
            tokens.append(var_or_keyword(token))
